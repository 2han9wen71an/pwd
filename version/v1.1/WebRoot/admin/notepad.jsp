...
<%
	request.setAttribute("title", "备忘录");
%>
	...
	<script>
		layui.use('table', function() {
			...
			table.render({
				...
				even: false,
				elem: '#notepad_list',
				url: 'notepad.do?method=notepad_list',
				cellMinWidth: 80,
				cols: [
					[{
						type: 'numbers'
			...
			form.on('switch(Status)', function(obj) {
				...
				$.post('notepad.do?method=notepad_status', {
					id: id,
					type: code
				}, function(data) {
		...
		function del(id) {
			$.post('notepad.do?method=notepad_del', {
				id: id
			}, function(data) {
				var res = JSON.parse(data);
				if(res['code'] == 200) {
	...
	<script type="text/javascript">
		layui.use('form', function() {
			...
			form.on('submit(formGet_inc)', function(data) {
				...
				$.post('notepad.do?method=notepad_inc', {
					content: content,
					title: title
				}, function(data) {
					var res = JSON.parse(data);
		...
		function search() {
			...
			layui.use('table', function() {
				...
				table.reload('notepad_list', {
					url: 'notepad.do?method=notepad_search',
					method: 'post',
					where: {
						type: v,
						content: content
